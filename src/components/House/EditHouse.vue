<template>
  <v-form ref="form" v-model="isFormValid">
    <v-card>
      <v-card-title>
        <label class="title">{{ $t('house.edit_house') }}</label>
      </v-card-title>
      <v-card-text>
        <v-container>
          <v-row>
...
export default {
  ...
  methods: {
    ...
    getHouse () {
      this.isLoading = true
      this.$proxy.get(`residential/${this.residentialId}/house/${this.houseId}`)
        .then((response) => {
    ...
    save () {
      ...
      this.$proxy.put(`residential/${this.residentialId}/house/${this.houseId}`, this.house)
        .then(() => {
          this.$emit('saved')
