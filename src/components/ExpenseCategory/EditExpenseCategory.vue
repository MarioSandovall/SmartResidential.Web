<template>
  <v-form ref="form" v-model="isFormValid">
    <v-card>
      <v-card-title>
        <label class="title">{{ $t('expense_category.edit_expense_category') }}</label>
      </v-card-title>
      <v-card-text>
        <v-container>
          <v-row>
...
export default {
  ...
  methods: {
    ...
    getCategory () {
      this.isLoading = true
      this.$proxy.get(`residential/${this.residentialId}/expense-category/${this.categoryId}`)
        .then((response) => {
    ...
    save () {
      this.isLoading = true
      this.$proxy.put(`residential/${this.residentialId}/expense-category/${this.categoryId}`, this.category)
        .then((response) => {
          this.$emit('saved')
